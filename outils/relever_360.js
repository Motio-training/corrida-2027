...
/* ---------------------------------------------------------------- données
   La lecture de la carte et le lancer de rayon en azimut vivent dans
   outils/carte.js : plan_360.js s'en sert aussi, et c'est la pièce dont
   dépendent toutes les hauteurs relevées — en garder deux copies, c'est
   se préparer à les voir diverger. */
const PI=Math.PI;
const C=require('./carte.js').charger({page:PAGE, gw:GW, portee:PORTEE});
const {BATS, pX, pZ, loDeX, laDeZ, visibilite, proches, GRILLE_CASE}=C;
/* --------------------------------------------------------- lecture d'image */
...
if(ARG.includes('--essai-geometrie')){
  ...
  C.remplacerBatiments([{i:0,k:'y',cx:0,cz:-20,ow:10,ol:10,aire:100,lv:0,ht:0,p:c,bb:[x0,x1,z0,z1]}]);
