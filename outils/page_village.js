...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    var n=0;
    C.fichiers.reduce(function(p,f){
      return p.then(function(){
        n++;
        if(txt) txt.textContent='Téléchargement de la 3D ('+n+' / '+C.fichiers.length+')…';
        if(jauge) jauge.style.width=Math.round(n/C.fichiers.length*100)+'%';
        return charger(f);
      });
